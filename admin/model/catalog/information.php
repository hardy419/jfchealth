...
class ModelCatalogInformation extends Model {
	...
	public function getNewsList($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "news n WHERE 1";
	...
	public function addNews($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "news SET sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "' , title = '" . $this->db->escape($data['title']) . "', description = '" . $this->db->escape($data['description']) . "', time='".date("d-m-Y")."'");

		$news_id = $this->db->getLastId(); 
		
		foreach($data['new_images'] as $value)
		{
			$this->db->query("INSERT INTO " . DB_PREFIX . "news_image SET news_id ='".$news_id."', sort_order = '" . (int)$value['sort_order'] . "', image = '" . $this->db->escape(html_entity_decode($value['image'], ENT_QUOTES, 'UTF-8')) . "'");
		}
	...
	public function editNews($news_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "news SET sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "' , title = '" . $this->db->escape($data['title']) . "', description = '" . $this->db->escape($data['description']) . "' WHERE news_id = '" . (int)$news_id . "'");
	...
	public function deleteNews($information_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "news WHERE news_id = '" . (int)$information_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "news_image WHERE news_id = '" . (int)$information_id . "'");

		$this->cache->delete('news');
	}
	
	public function getNews($information_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "news n WHERE news_id = '" . (int)$information_id . "'");
